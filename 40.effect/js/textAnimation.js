...
function textAni(cssSelector,option){
  ...
  //제이쿼리문서객체.get(0) => 실항하게되면 제이쿼리문서객체를 자바스크림트 문서객체로 변환하여 리턴한다.
  //문제데이터들은 배열처럼 구성이 되어있다.
  ...
  for(let i =0 ; i<$(cssSelector+" b").length ; i++){
    $(cssSelector+" b:nth-of-type("+(i+1)+")").css("animation",aniName+" "+aniamtionDuration+"s ease "+(aniamtionTerm*i)+"s forwards")
  }
} 